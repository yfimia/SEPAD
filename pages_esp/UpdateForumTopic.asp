...
<%
/*
    Esta página se usa para modificar un tema a los foros de 
    discusión. 
...
<%  
  ...
  if (uid == Session("uid"))
    {    
      ...
      oConn.Open(filePath);
      oRec.Open("SELECT * FROM ForumTopics WHERE id=" + topicID, oConn, 3, 3);
      if (oRec.EOF == false) {
       var mititulo = oRec.Fields.Item("Titulo").value;
       var midescripcion = oRec.Fields.Item("Descripcion").value;
       var miactivo = oRec.Fields.Item("Activo").value;
      }
      else
      {
       var mititulo = '';
  ...
  <tr> 
    <td>
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <td class="MessageTR1"> 
            <table border="0" cellspacing="0" cellpadding="0" width="100%">
              <tr> 
                ...
                <td width="99%"> 
                  <input type="text" id="Mtitulo" name="Mtitulo" class="Edit" size="106" maxlength="255" value="<%=mititulo%>">
                </td>
              </tr>
            </table>
          </td>
        ...
        <tr> 
          <td class="MessageTR1"> 
            <textarea name="Mbody" ID="Mbody" rows="12" class="TextArea"><%=midescripcion%></textarea>
          </td>
        </tr>
        <tr> 
          <td class="MessageTR1"> 
...
<FORM ID=Mail NAME=Mail METHOD=POST ACTION="AceptaUpdateForumTopic.asp?uid=<%=uid%>&course=<%=CourseInUse%>&pn=<%=pagenumber%>&courseName=<%=courseNameInUse%>&topicID=<%=topicID%>" >
