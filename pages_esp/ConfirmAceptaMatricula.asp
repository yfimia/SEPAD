          ...
          for (i=1; i<=j; i++) 
            {
              ...
              oConn.Open( filePath );
              oRec.Open("SELECT     Mod_Matricula.* FROM Mod_Matricula WHERE (idmatricula = " + IDMA[i] + ")",oConn,3,3);
                        
              oRec1.Open("SELECT * FROM TMP_Matriculas_Cursos",oConn,3,3);
              
              while (!oRec.EOF) {
                oRec1.AddNew();
                oRec1.Fields.Item("user").Value   = IID;
                oRec1.Fields.Item("modulo").Value   = oRec.Fields.Item("idmodulo").Value;
                oRec1.Update();
                oRec.Move(1);
