...
<!-- #include file='inc/ConfirmAgregaGroupCur.inc' -->
<%
      ...
      while (oRec.EOF == false)
       ...
       if (j >= 1)
         ...
         if (k >= 1)
          {  
           oConn.Open(filePath);
           oRec.Open("select * from Grupos where (ID = " + Session("admgrupo") + ")",oConn,3,3);
 ...
 Response.Redirect('MtrGrpCour.asp?uid=' + uid + '&course=' + Session("admcourse") + '&coursename=' + Session("admcourseName"));
...
function muOnclick()
 {
   location = 'MtrGroupCur.asp?uid=<%=uid%>&course=<%=Session("admcourse")%>&coursename=<%=Session("admcourseName")%>' ;
 }
 
//-->
</SCRIPT>

 <center><b><%=GruposMatriculados%></b></center>
