...
<%    
      ...
      oComm.CommandText = "SELECT * FROM grupos where (ID = " + Session("admgrupo") + ")";
      oRec = oComm.Execute();
...
%>            
          ...
          <td width="20%" class="<%=clase%>"><a href="userDetails.asp?uid=<%=uid%>&user=<%=oRec.Fields.Item("ID").value%>"><%=oRec.Fields.Item("name").value%></a></td>
    ...
    <td class="ToolBar">
      <table border="0" cellspacing="0" cellpadding="4" class="ToolBar">
        <tr> 
          ...
          <td>|</td>
          <td><a href="verEliminaralumnado.asp?uid=<%=uid%>&first=<%=last%>" class="ToolLink">&nbsp;Próximos&nbsp;<%=SHOW_CANT%>&nbsp;</a></td>
        </tr>
      </table>
    </td>
  </tr>
