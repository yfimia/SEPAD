...
function getResp(resp,pes,po)
  {
   ...
   while (resp != "")
   {
    ...
    //Response.Write("<br> rArr " + rArr[i] + "<br>");
 ...
 function isokrArr(cad)
  { var i;
    var result = false;
    var tmp1,tmp2;
   //Response.Write("rArrCant " + rArrCant + "<br>");
            ...
            if (ev[i] != 0) 
             {
              ...
              oRec.Fields.Item("Response").value = "";
              for(j=0; j < Session("exer").items.cant[Session("Cuales").items[i]]; j++) {
                cad = "O" + Session("exer").items.id[Session("Cuales").items[i]] + "#" + j;
                oRec.Fields.Item("Response").value  += Request.Form.Item(cad) + "|||";
              }  
              oRec.Update();      
             }
            else
...
<%
 ...
 for (var j = 0;j <= Session("Cantidad");j++)
  {
   var i = 0; 
   ok = 0;
   resOpt = 0; 
   ...
   //Response.Write(Session("exer").items.cant[Session("Cuales").items[j]]);
     ...
     switch (Session("exer").items.type[Session("Cuales").items[j]])
      {
	    ...
	    case "2":
	     {
	      //Enlazar...
	      getResp1(response,pesos,i);
				//Response.Write(rArr[i] + " - " + req.toUpperCase() + "<br>"); 
				         
	      if (req.toUpperCase() == rArr[i])               
           { //Response.Write(" " + pe);
            ok = ok + parseInt(pArr[i],10);
            bien = bien + 1;
