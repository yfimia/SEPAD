...
<%   var  filePath = Application("dataPath");   
   ...
   var Count         = 0;   
   var SQL           = "SELECT Top " + SHOW_CANT + " Mensajeria.ID, Priority, Readed, [From], Subject, Date, Usuarios.FullName " +
                       "FROM Usuarios INNER JOIN Mensajeria ON Usuarios.ID = Mensajeria.[To]  " +  where + " ORDER BY Date DESC, [From], Readed DESC, Priority DESC";
%>
<html>
<head>
...
//     event.srcElement.parentElement.parentElement.style.fontWeight  = "lighter";
     abreVentana('',700,484,'ViewMsg.asp?uid=<%=uid%>&ID=' + id + '&Flag=1&adm=1', 'no', 'no')
}  
