        ...
        */
        ...
        if (operation == 1){
            ...
            if (Request.QueryString.Item("sd").count != 0){
              ...
              if ((Session("PermissionType") == ADMINISTRATOR) || (Session("owner") = Session("userID")) || (Session("cordinador") = Session("userID"))){
        ...
        oRec.Open("SELECT Salas.ID,Salas.Objetivo, Salas.Moderador,Salas.Publica, Salas.Nombre, count(*) As Cantidad " + 
                  "FROM Salas, ChatOnlineUsers,SalasCurso " + 
                  "WHERE (Salas.ID = ChatOnlineUsers.Sala) and (Salas.ID = SalasCurso.Sala) and (SalasCurso.Curso = " + Session("course") + ")" + 
                  ...
                  "FROM Salas,SalasCurso " + 
                  "WHERE (SalasCurso.Curso = " + Session("course") + ") and (Salas.ID = SalasCurso.Sala) and (NOT (Salas.ID IN (SELECT sala FROM ChatOnlineUsers)))", oConn, 3, 3);    
%>
<HTML>
<HEAD>
  <title>Listado de las sala de charlas pertenecientes al módulo actual</title>
      ...
      <td class="ToolBar">
        <table border="0" cellspacing="0" cellpadding="0" class="ToolBar">
          <tr> 
            ...
            <td class="HeaderTable">Lista de salas del charlas del módulo</td>
          </tr>
        </table>
      </td>
    </tr>
        ...
        <table border="0" cellspacing="0" cellpadding="4" class="ToolBar">
          <tr> 
            <%
              ...
              //if ((Session("userID") == fullCourse.owner) || (Session("userID") == Session("cordinador")) || (Session("PermissionType") == ADMINISTRATOR) )  { 
            %>
            <td nowrap>
              <a href="newChatAddModuleRoom.asp?uid=<%=uid%>&course=<%=idcursoactual%>&courseName=<%=nombrecursoactual%>" class="ToolLink">
                &nbsp;Agregar nueva sala&nbsp; <%=Session("cordinador")%> _ <%=Session("UserID")%>
              </a>
            </td>
            <td>|</td>
            <%
            ...
            %>
            <td>
              <a href="newChatModuleRoomList.asp?pn=<%=pagenumber + 1%>" class="ToolLink">
                &nbsp;Próximas&nbsp;<%=SHOW_CANT%>&nbsp;
              </a>
            </td>
          </tr>
          ...
          <tr> 
            ...
            <td width="1%" class="<%=clase%>">           
              <%
                if ((oRec.Fields.Item("Moderador").value == Session("userID")) || (Session("PermissionType") == ADMINISTRATOR)){
              %>
              <a href="newChatModuleRoomList.asp?uid=<%=uid%>&course=<%=idcursoactual%>&courseName=<%=nombrecursoactual%>&pn=<%=pagenumber%>&op=1&sd=<%=oRec.Fields.Item("ID").value%>">
              ...
              <table border="0" cellspacing="1" cellpadding="1" class="ToolBar" width="100%">
                <tr  valign=top>
                  <td colspan=5  align=center  valign=top>
                    ...
                    %>    
                    <a class="numbers" href="newChatModuleRoomList.asp?pn=<%=i%>">
                      <b><%=i + 1%>&nbsp;</b>
                    </a>  
                    <%
                        i = i + 1;
