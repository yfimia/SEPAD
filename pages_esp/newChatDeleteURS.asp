      ...
      if (Request.Form("cantidad").Count != 0){
        for (k = 1; k <= Request.Form("cantidad"); k++){
          if ((Request.Form("nombre" + k).Count != 0) && (Request.Form("nombre" + k) != "")){
            oRec.Open("DELETE From ChatCanSpeak WHERE (Usuario = " + Request.Form("nombre" + k) + ") and (sala = " + sala + ")", oConn, 3, 3);
          }  
        }
      }  
  
      oRec.Open("SELECT ChatCanSpeak.Usuario, Usuarios.Name, Usuarios.FullName, Usuarios.Sexo, Usuarios.Email " + 
                "FROM Usuarios INNER JOIN ChatCanSpeak ON Usuarios.ID = ChatCanSpeak.Usuario " + 
                "WHERE ChatCanSpeak.Sala = " + sala, oConn, 3, 3);
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr> 
      <td colspan=3 class="ToolBar" align="center">
        <b>Listado de usuarios que pueden hablar</b>
      </td>
    </tr>
    <tr> 
      <td> 
        <form id="delusuarios" name="delusuarios" action="newChatDeleteURS.asp?salaid=<%=sala%>&pn=<%=pagenumber%>" method=post>      
              ...
              <td>
                <table border="0" cellspacing="0" cellpadding="4" class="ToolBar">
                  <tr> 
                    ...
                    <td>|</td>
                    <td>
                      <a href="newChatDeleteURS.asp?salaid=<%=sala%>&pn=<%=pagenumber + 1%>" class="ToolLink">
                        &nbsp;Próximos&nbsp;<%=SHOW_CANT%>&nbsp;
                      </a>
                    </td>
                  </tr>
                ...
                <table border="0" cellspacing="0" cellpadding="0" class="ToolBar" width="100%">
                  <tr>
                    <td colspan=5  align=center>
                      ...
                      %>    
                      <a class="numbers" href="newChatDeleteURS.asp?salaid=<%=sala%>&pn=<%=i%>">
                        <b><%=i + 1%>&nbsp;</b>
                      </a>  
                      <%
                          i = i + 1;
