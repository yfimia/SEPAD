  ...
  if (oRec.EOF == false){
    ...
    if ((Session("userID") == salamoderador) || (Session("PermissionType") == ADMINISTRATOR)){
      ...
      if (Request.Form("cantidad").Count != 0){
        oRec.Open("SELECT * From ChatCanEnter", oConn, 3, 3);    
        for (k = 1; k <= Request.Form("cantidad"); k++){
          if ((Request.Form("nombre" + k).Count != 0) && (Request.Form("nombre" + k) != "")){
            oRec1.Open("SELECT * From ChatCanEnter WHERE (Usuario = " + Request.Form("nombre" + k) + ") and (sala = " + sala + ")", oConn, 3, 3);
            if (oRec1.EOF == true){
              oRec.AddNew();
          ...
          DoMyAction = "<script language=jscript>window.parent.frames(2).location = 'newChatDeleteURE.asp?salaid=" + sala + "&pn=0'; </script>";  
        }
        oRec.Update();
        oRec.Close(); 
      }  
...
<body text="#000000" style="overflow-y:auto">
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr> 
      <td> 
        <form id="usuarios" name="usuarios" action="newChatAddURE.asp?salaid=<%=sala%>&pn=<%=pagenumber%>" method=post>
              ...
              <td>
                <table border="0" cellspacing="0" cellpadding="4" class="ToolBar">
                  <tr> 
                    ...
                    <td>|</td>
                    <td>
                      <a href="newChatAddURE.asp?salaid=<%=sala%>&pn=<%=pagenumber + 1%>" class="ToolLink">
                        &nbsp;Próximos&nbsp;<%=SHOW_CANT%>&nbsp;
                      </a>
                    </td>
                  </tr>
                ...
                <table border="0" cellspacing="0" cellpadding="0" class="ToolBar" width="100%">
                  <tr>
                    <td colspan=5  align=center>
                      ...
                      %>    
                      <a class="numbers" href="newChatAddURE.asp?salaid=<%=sala%>&pn=<%=i%>">
                        <b><%=i + 1%>&nbsp;</b>
                      </a>  
                      <%
                          i = i + 1;
