...
%>
        <tr>
           ...
           <table width="100%" border="0" cellspacing="5" cellpadding="1">
              <tr>
                <td><h6><%=stmp1%><a href='javascript:<%=MATRICULAS_PAGE%>' class="ModifyBtnLnk">Matricular.</a></h6></td>
              </tr>
            </table>  
          </td>
        </tr>
...
%>          
<%    } 
       if ((Session("userID") == Session("courseOwner")) || (Session("userID") == Session("cordinador")) || (Session("PermissionType") == ADMINISTRATOR) || (isUserInGroupByID(Session("userID"), Session("claustro")))) { //si es tutor
%>              
          <td  width="1%" >|</td>
...
%>          
          <td  width="1%" >|</td>
          <td  width="1%" ><a href="javascript:abreVentana(<%=stmp5%>,700,400,'moduloAdm/main.asp?uid=<%=uid%>&course=<%=Session("course")%>&courseName=<%=Session("courseName")%>&courseOwner=<%=fullCourse.owner%>', 'no', 'no')" class="ToolLink">&nbsp;<%=stmp6%>&nbsp;</a></td>
<%
       }
%>          
          <td  width="10%" ></td>
...
/*   else
      if ((Session("state") == MOD_ACA_ININSCRIPTION) || (Session("state") == MOD_ACA_INCOURINSC))        
       { //Si es un usuario del sistema pero no ha matriculado el curso...
         //Que matricule el curso primero...
%>
        <tr>
           <td>
           <table width="100%" border="0" cellspacing="5" cellpadding="1">
              <tr>
                <td> 
                  <h6><%=stmp7%><a href="javascript:abreVentana(<%=stmp8%>,250,110,'matriculatemp.asp?uid=<%=uid%>&course=<%=Request.QueryString("course")%>')" class="ModifyBtnLnk"><%=stmp9%></a><%=stmp%></h6> 
                </td>
              </tr>
            </table>  
          </td>
