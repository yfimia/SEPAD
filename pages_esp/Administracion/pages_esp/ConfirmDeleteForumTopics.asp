          ...
          for (i=1; i<=j; i++) 
            {
              ...
              oComm.CommandText = "delete  from ForumTopics where (ID = " + fID[i] + ")";
              oRec = oComm.Execute();
              oConn.Close();          
            }
          Response.Write("<center><font color=red>Temas eliminados satisfactoriamente</font><br></center>");  
        }  
      Session("UserList") = null;
      Session("Conection") = null;
      Session("Command") = null;
        
      Response.Redirect("DeleteForumTopics.asp?uid=" + uid);      
      
      Response.Write ('<script languaje=javascript>function pclick(){window.location="DeleteForumTopics.asp";}</script>');      
