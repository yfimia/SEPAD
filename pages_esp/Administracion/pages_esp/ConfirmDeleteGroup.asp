...
<!-- #include file='inc/confirmdeletegroup.inc' -->
<%
...
%>
        <tr> 
<%
              if ( empty("SELECT ID FROM Modulo WHERE (claustro = " + Request.Form("ID" + i) + ") or (grupo =  " + Request.Form("ID" + i) + ")") ) {

                oComm.CommandText = "delete  from Grupos where (ID = '" + Request.Form("ID" + i) + "')";
                oRec = oComm.Execute();
%>
...
//      Response.Redirect("Deletegroup.asp?uid=" + uid);      
      
      Response.Write ('<script languaje=javascript>function pclick(){window.location="Deletegroup.asp?uid=' + uid + '&first=' + first + '";}</script>');
