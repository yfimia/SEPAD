      ...
      if (Request.Form("cantidad").Count != 0){
        for (k = 1; k <= Request.Form("cantidad"); k++){
          if ((Request.Form("nombre" + k).Count != 0) && (Request.Form("nombre" + k) != "")){
            oRec.Open("DELETE From ChatCanEnter WHERE (Usuario = " + Request.Form("nombre" + k) + ") and (sala = " + sala + ")", oConn, 3, 3);
          }  
        }
      }  
  
      oRec.Open("SELECT ChatCanEnter.Usuario, Usuarios.Name, Usuarios.FullName, Usuarios.Sexo, Usuarios.Email " + 
                "FROM Usuarios INNER JOIN ChatCanEnter ON Usuarios.ID = ChatCanEnter.Usuario " + 
                "WHERE ChatCanEnter.Sala = " + sala, oConn, 3, 3);
...
<HEAD>
  ...
  <script language=jscript>
    function Elimina(){
      for (l = 1; l <= delusuarios.cantidad.value; l++){
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr> 
      <td colspan=3 class="ToolBar" align="center">
        <b>Listado de usuarios que pueden entrar</b>
      </td>
    </tr>
    <tr> 
      <td> 
        <form id="delusuarios" name="delusuarios" action="newChatDeleteURE.asp?salaid=<%=sala%>&pn=<%=pagenumber%>" method=post>      
...
%>
            <tr> 
              <td width="5%" class="<%=clase%>">
                ...
                <input type="hidden" id=nombre<%=i%> name=nombre<%=i%> value="<%=oRec.Fields.Item("Usuario").value%>">
              </td>
            ...
            <tr>
              <td>
                <table border="0" cellspacing="0" cellpadding="4" class="ToolBar">
                  <tr> 
                    <td nowrap>
                      <a href="javascript:Elimina()" class="ToolLink">
                        &nbsp;Eliminar&nbsp;
                      </a>
                    </td>
                    <td>|</td>
                    ...
                    <td>|</td>
                    <td>
                      <a href="newChatDeleteURE.asp?salaid=<%=sala%>&pn=<%=pagenumber + 1%>" class="ToolLink">
                        &nbsp;Próximos&nbsp;<%=SHOW_CANT%>&nbsp;
                      </a>
                    </td>
                  </tr>
                ...
                <table border="0" cellspacing="0" cellpadding="0" class="ToolBar" width="100%">
                  <tr>
                    <td colspan=5  align=center>
                      ...
                      %>    
                      <a class="numbers" href="newChatDeleteURE.asp?salaid=<%=sala%>&pn=<%=i%>">
                        <b><%=i + 1%>&nbsp;</b>
                      </a>  
                      <%
                          i = i + 1;
