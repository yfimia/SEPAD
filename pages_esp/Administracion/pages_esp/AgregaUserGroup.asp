...
<form name=Delnews action="ConfirmAgregaUserGroup.asp?uid=<%=uid%>&first=<%=last%>" method="post">
...
<%
      oComm.CommandText = "SELECT * FROM grupos where (ID <> 60)";
      oRec1 = oComm.Execute();    
...
%>        
          <option  
<%
          if (oRec1.Fields.Item("ID").Value == Request.QueryString.Item("group")) {
...
%>          
          value="<%=oRec1.Fields.Item("ID").Value%>"><%=oRec1.Fields.Item("Name").Value%></option>
<%
          oRec1.Move(1);
        }
%>        
    ...
    <td>
      <table border="0" cellspacing="0" cellpadding="4" class="ToolBar">
        <tr> 
          ...
          <td>|</td>
          <td><a href="AgregaUserGroup.asp?uid=<%=uid%>&first=<%=last%>" class="ToolLink">&nbsp;Próximos&nbsp;<%=SHOW_CANT%>&nbsp;</a></td>
        </tr>
      </table>
    </td>
  </tr>
