...
<%
 ...
 var   recRange = 10
 // Get table name
 var  tablename = "[Mensajes_Enviados]";
    ...
    strsql = "select Mensajes_Enviados.ID, Priority, Readed, [FromID], [To], Subject, Date   from " + 
             " Mensajes_Enviados  " ;
    dbwhere = " ([FromID]="+ Session("UserID")+") ";
   ...
   if (Request.Form.Count > 0)  
     {
      var SQL        = "DELETE FROM Mensajes_Enviados WHERE ";
...
<form id="Messages" method="POST" action="sent_mesages.asp?uid=<%=uid%>">
    ...
    <td> 
      <table width="100%" cellpadding="0" cellspacing="1" border="0">
        <tr> 
          ...
          <td width="1%" class="MessageInfo">&nbsp;</td>
          <td width="30%" class="MessageInfo"><b><a  class="ToolLink" href="sent_messages.asp?uid=<%=uid%>&order=<%= Server.URLEncode("To") %>">Para</a></b></td>
          <td width="48%" class="MessageInfo"><b><a  class="ToolLink" href="sent_messages.asp?uid=<%=uid%>&order=<%= Server.URLEncode("Subject") %>">Asunto</a></b></td>
          <td width="20%" class="MessageInfo"><b><a  class="ToolLink" href="sent_messages.asp?uid=<%=uid%>&order=<%= Server.URLEncode("Date") %>">Enviado</a></b></td>
...
%>
        ...
        <tr  <%=bold%> class="<%=trclass%>"  valign="middle"> 
          ...
          <td align=center width="30%"><%=Vinculo%><%=oRec.Fields("To").Value %></a></td>
...
%>
	<center>
 	 <span class="txtboldy"><a href="sent_messages.asp?uid=<%=uid%>&start=<%=PrevStart%>">&lt;&lt;&nbsp;Prev</a></span>
<%
	}
...
<% 			    } else {
%>
	<span class="txtboldy"><strong><a href="sent_messages.asp?uid=<%=uid%>&start=<%=x%>"><%=y%></a></strong></span>
<%
			    }
				x = x + displayRecs;
				y = y + 1;
	...
	<span class="txtboldy"><strong><a href="sent_messages.asp?uid=<%=uid%>&start=<%=x%>"><%=y%>-<%=y+recRange-1%></a></span>
<%			    }
                else {
					ny=Math.floor((totalRecs-1)/displayRecs)+1;
						if (ny == y) {
%>
	<span class="txtboldy"><a href="sent_messages.asp?uid=<%=uid%>&start=<%=x%>"><%=y%></a></span>
<%					    } else {
%>
	<span class="txtboldy"><a href="sent_messages.asp?uid=<%=uid%>&start=<%=x%>"><%=y%>-<%=ny%></a></span>
<%
					    }
				}
	...
	if (! oRec.EOF) {
		...
		isMore = true; 
%>
	<span class="txtboldy"><a href="sent_messages.asp?uid=<%=uid%>&start=<%=NextStart%>">Next&nbsp;&gt;&gt;</a></span>
<%  }
	else {
	  isMore = false;
	}  
...
function onDblclick( id ) {    
     ...
     abreVentana('',800,420,'ViewSentMsg.asp?uid=<%=uid%>&ID=' + id + '&Flag=1&adm=1', 'no', 'no')
}  

    
function onDeleteMessage() {      
     	Messages.all("flag").value = 0;
     	Messages.action = "sent_messages.asp?uid=<%=uid%>";
     	Messages.submit();
}
