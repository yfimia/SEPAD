...
<%    
      ...
      oComm.CommandText = "SELECT Usuarios.*, Usuarios.Name " +
                          "FROM Usuarios INNER JOIN Grupos_de_Usuarios ON Usuarios.ID = Grupos_de_Usuarios.[User] " +
                          "WHERE (((Grupos_de_Usuarios.[Group])=" + Session("admcursogrupo") + "))ORDER BY Usuarios.Name";
...
<%      
   DestroyAdoObjects( oConn, oRec );       
    ...
    {
      Response.Redirect("../errorpage.asp?tipo=Error&short=" + Session("admcursocordinador") + DONT_HAS_PERMISON_SHORT  + "&desc=" + DONT_HAS_PERMISON_TEXT);         
      }
%>
