...
<%
          ...
          for (i=1; i<=j; i++) 
            {
              ...
              oComm.CommandText = "delete  from Seminarios where (ID = " + IDTrab[i] + ")";
              oRec = oComm.Execute();
              oConn.Close();          
  ...
  Response.Redirect('AdminUpLoadedFiles.asp?uid=' + uid + "&courseOwner=" + Request.QueryString.Item("courseOwner")); 
