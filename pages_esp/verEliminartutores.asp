...
<!-- #include file='inc/verEliminartutores.inc' -->
<%
...
<%  
  ...
  if ((Session("PermissionType") == ADMINISTRATOR) || (Session("PermissionType") == PUBLICATOR))
    { var adm = (Session("PermissionType") == ADMINISTRATOR);
%>
...
<form name=Delnews action="ConfirmDeleteTutor.asp?uid=<%=uid%>" method="post">
...
<%    
      ...
      oComm.CommandText = "SELECT * FROM grupos where (Name = 'Tutores_Curso_" + coursename + "')";
      oRec = oComm.Execute();
...
%>          
          ...
          <td>|</td>
          <td><a href="verEliminartutores.asp?uid=<%=uid%>&course=<%=course%>&coursename=<%=coursename%>&first=<%=last%>" class="ToolLink">&nbsp;<%= TEXT8 %>&nbsp;<%=SHOW_CANT%>&nbsp;</a></td>
