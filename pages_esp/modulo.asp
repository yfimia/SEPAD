...
<%
  if ((Session("userID")==GUEST_USER)  && ((state == MOD_ACA_ININSCRIPTION) || (state == MOD_ACA_INCOURINSC))) {  
...
<% 
  ...
  if (isowner) {       
    if ((state == MOD_ACA_ININSCRIPTION) || (state == MOD_ACA_INCOURINSC) || (state == MOD_ACA_DEVELOP)) {       
%>      
          <td class="Separador" width="1%">|</td>
<%  
    }
%>
          <td width="1%"><a href="javascript:abreVentana('ModalidadAdm',850,600,'ModAdm/main.asp?uid=<%=uid%>&modulo=<%=Session("modulo")%>&courseName=<%=Session("moduloName")%>', 'yes', 'yes')" class="ToolLink">&nbsp;Administrar&nbsp;</a></td>
<%
  }
%>
        <td width="10%">
...
%>        
   ...
   <%  
      switch (parseInt(state)) {
       ...
       case MOD_ACA_FREE : {%>Libre<% break;}
       case MOD_ACA_DEVELOP : {%>En Desarrollo<% break;}
...
<%
  for (i = 0; i < courses.length; i++) {
    if ((courses[i].state == MOD_ACA_FREE) || (Session("state") == MOD_ACA_FREE) || (courses[i].state != MOD_ACA_NOVISIBLE) || (isowner) || (courses[i].owner == Session("userid")) ) { 
%>  	
<tr>
...
<%  
  if (((courses[i].owner == Session("userID")) || (isowner) || (valid && (courses[i].state == MOD_ACA_INCOURSE) && ((state == MOD_ACA_INCOURSE) || (state == MOD_ACA_INCOURINSC)))) 
      || ((courses[i].state == MOD_ACA_FREE) || (state == MOD_ACA_FREE))) {
...
%>        
        ...
        <a  href="javascript:abreVentana('CourseAdm',800,600,'moduloAdm/main.asp?uid=<%=uid%>&course=<%=courses[i].ID%>&courseName=<%=courses[i].Name%>&courseOwner=<%=courses[i].owner%>&cordinador=<%=Session("cordinador")%>&modulo=<%=Session("modulo")%>&moduloName=<%=Session("moduloName")%>&state=<%=Session("state")%>&grupo=<%=Session("grupo")%>&claustro=<%=Session("claustro")%>', 'yes', 'yes')" ><img title="Administrar" src="../images/<%=Session("skin")%>/admin.gif"  width="20" height="20" border="0" ></a>
<%
  }
%>           
