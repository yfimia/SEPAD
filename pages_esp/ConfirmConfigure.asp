...
if (Session("PermissionType") == ADMINISTRATOR)
    {
      ...
      /*if (Request("AutoMtr").Count > 0 )
        Application("AutoMtr")   = "1";     
      else   */
        Application("AutoMtr") = "0";
    ...
    if (oXmlDoc.documentElement.selectSingleNode("/SysConfig/General/AutoMtr") != null)
      oXmlDoc.documentElement.selectSingleNode("/SysConfig/General/AutoMtr").text =   Application("AutoMtr");
    else {
      var newNode = oXmlDoc.createNode(1, "AutoMtr", "");
      var XmlNode = oXmlDoc.documentElement.selectSingleNode("/SysConfig/General");
      XmlNode.appendChild(newNode);
    }
        

    oXmlDoc.save('D:\\sepad\\webapp\\XML\\config.xml');
