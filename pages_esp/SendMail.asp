...
<% 
 ...
 function Init()
  {
   ...
   if ( MailTo.charAt(0) == '1') //Un usuario
   { 
     ToID[0] = MailTo.substr(2);
   }
   else
   if ( MailTo.charAt(0) == '2' ) //Un Subgrupo Tutoreado...
   {
    //********** Patch ( 30/6/2004 ) **************
     	var oConn;
     	var oRec;
     	...
     	oConn = MakeConnection( oConn, filePath );      
     	Sql   = "SELECT Usuario FROM UsuariosSubGrupo WHERE SubGrupo = " + MailTo.substr(2);           
 
     	oRec = Query( Sql, oRec, oConn  );		
     	while (oRec.EoF == false) 
       	{  
       	   Response.Write( oRec.Fields("Usuario").Value + ":" );   
           ToID[i] = oRec.Fields("Usuario").Value;
           i++;
       	   oRec.Move(1);
       	}
 ...
 var Sql1     = "SELECT * FROM Mensajes_Enviados";
 ...
 for (i=0;i <= ToID.length-1;i++)
  {
   ...
   oRec1.Fields.Item("FromID").value     = Session("userid"); 
   oRec1.Fields.Item("Readed").value     = false;
   oRec1.Fields.Item("To").value       = parseInt(ToID[i], 10); 
   oRec1.Fields.Item("ToText").value       = MailToText; 
   oRec1.Fields.Item("Subject").value  = MailSubject; 
   oRec1.Fields.Item("Priority").value = MailPriority;
   oRec1.Fields.Item("Content").value  = MailBody;
   oRec1.Fields.Item("Date").value     = getDate();      
