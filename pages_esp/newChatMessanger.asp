...
<%
  ...
  if (oRec.EOF == false){
    ...
    if ((Request.Form("msgcolor").Count == 0) || (Request.Form("msgcolor") == "")){
      ...
      if (oRec.EOF == false){
       ...
       usercolor = oRec.Fields.Item("Color").value + "";
        ...
        if (usernegrita == "undefined"){
          usernegrita = true;
        }
        if (usercursiva == "undefined"){
          usercursiva = false;
        }
        if (usersubrayada == "undefined"){
          usersubrayada = false;
        }
        oRec.Close();
      }
    ...
    if ((salapublica) || (salamoderador == Session("userID")) || (oRec.EOF == false) || (Session("PermissionType") == ADMINISTRATOR)){
      ...
      //Response.Write(Request.Form("refreshtime"));
      if (Request.Form("refreshtime").Count != 0){
        Session("ChatTimeToRefresh") = Request.Form("refreshtime") * 1;
        //Response.Write("ok " + Session("ChatTimeToRefresh"));
      }  
              ...
              <table  border=0 cellpadding=0 cellspacing=0>
                <tr>
                  ...
                  <td  nowrap>
                    &nbsp;
                    Actualizar cada:
                    <select name=refreshtime ID=refreshtime title="Tiempo de actualizacion en segundos">
                      <option <%if (Session("ChatTimeToRefresh") * 1 ==  5){%>selected<%}%> value=10>5s</option>
                      <option <%if (Session("ChatTimeToRefresh") * 1 == 10){%>selected<%}%> value=10>10s</option>
                      <option <%if (Session("ChatTimeToRefresh") * 1 == 15){%>selected<%}%> value=15>15s</option>
                      <option <%if (Session("ChatTimeToRefresh") * 1 == 25){%>selected<%}%> value=25>25s</option>
                      <option <%if (Session("ChatTimeToRefresh") * 1 == 35){%>selected<%}%> value=35>35s</option>
                      <option <%if (Session("ChatTimeToRefresh") * 1 == 45){%>selected<%}%> value=45>45s</option>
                      <option <%if (Session("ChatTimeToRefresh") * 1 == 60){%>selected<%}%> value=60>60s</option>
                      <option <%if (Session("ChatTimeToRefresh") * 1 == 75){%>selected<%}%> value=75>75s</option>
                      <option <%if (Session("ChatTimeToRefresh") * 1 == 90){%>selected<%}%> value=90>90s</option>
                    </select>
                  </td>
                </tr> 
              </table>
